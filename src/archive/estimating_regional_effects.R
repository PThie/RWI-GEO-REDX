estimating_regional_effects <- function(
    ...
    # loop through all years/ quarters

    results_overall_list <- list()
    for (time_fe in  time_fes) {
        ...
        if (time_fe != "e_year_mon") {
            ...
            periods <- unique(housing_data[[time_fe]])

            # time label
            time_label <- helpers_regional_effects_change_settings(time_period = time_fe)[["time_label"]]

            #--------------------------------------------------
            # loop through all periods to get period-specific estimates

            for (period in periods) {
                #--------------------------------------------------
                # subset data to year/quarter

                est_data <- housing_data |>
                    dplyr::filter(!!rlang::sym(time_fe) == period)
                ...
                regional_coef <- regional_coef |>
                    dplyr::rename(!!time_label := period)

                #--------------------------------------------------
                # store results

                results_list[[paste0(time_label, "-", period)]] <- regional_coef
            }
        ...
        # combine all periods
        results_overall_list[[time_label]] <- results

        #--------------------------------------------------
        # export findings

        openxlsx::write.xlsx(
            results_overall_list[[time_label]] |> as.data.frame(),
            file.path(
